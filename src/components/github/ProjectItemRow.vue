...
<template>
  <button type="button" :class="ui.row()" @click="open">
    <span :class="ui.icon()">
      <BaseIcon name="circle-outline" size="sm" />
    </span>
    <div :class="ui.main()">
      <p :class="ui.title()">#{{ item.number }} {{ item.title }}</p>
      <div :class="ui.meta()">
        <span :class="ui.chip()">{{ item.statusName }}</span>
        <span v-if="item.repoName" :class="ui.chip()">{{ item.repoName }}</span>
