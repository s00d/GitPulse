...
const ui = computed(() =>
  tv({
    slots: {
      ...
      stars: "font-medium text-amber-600 dark:text-amber-400",
...
<template>
  <button type="button" :class="ui.card()" @click="open">
    <p :class="ui.name()">{{ props.repo.full_name }}</p>
    <div :class="ui.meta()">
      <span :class="ui.stars()">★ {{ compactCount(props.repo.stargazers_count) }}</span>
