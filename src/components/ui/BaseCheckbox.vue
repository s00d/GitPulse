...
const props = withDefaults(
  defineProps<{ disabled?: boolean; size?: UiControlSize; label?: string }>(),
...
const checkTv = tv({
  slots: {
    ...
    box: `grid place-items-center rounded border border-slate-300 bg-white transition-all ${uiMotion.base} data-[checked=true]:border-indigo-600 data-[checked=true]:bg-indigo-600 dark:border-slate-700 dark:bg-slate-900 dark:data-[checked=true]:border-indigo-500 dark:data-[checked=true]:bg-indigo-500`,
...
const ui = computed(() => checkTv({ size: props.size }));
</script>
